...
def gui(config, config_path, down):
    ...
    while True:             # Event Loop
        ...
        window.Element('_x_').Update(('Il servizio è già attivo :)',
                                      'Il servizio è disattivato :(')[down])
        window.Element('_B_').Update(('Stop Backup', 'Start Backup')[
   ...
   # data modifica in perch->
    ...
    if bool:
        ...
        name_dir = datetime.fromtimestamp(
            data_mod).strftime("Primo_di_Oggi(%d%m)")
    else:  # ORARIO_dst1,ORARIO_dst2,ORARIO_dst3
        name_dir = datetime.fromtimestamp(
            data_mod).strftime("Orario(%H_%M)Data(%m-%d)")
...
def stop(config, config_path):
    ...
    print("File creati e backup fermo")
...
def main():
    ...
    if not os.path.isfile(config_path):
        ...
        sg.popup_auto_close("ERRORE! Devi impostare il backup con Settings!",
                            button_color="red", auto_close_duration=10)
        sys.exit(69)
